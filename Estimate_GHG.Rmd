...
The individual routes are connected to form a loop based on the distance from the end point of one route to the start point of the next. The distance to and from the AD is also added
```{r}
...
run_TSP() is used to get the VMT of the cluster for collection/hauling (above)
## Set up constants
```{r}
...
These values are dependent on the amount of FW diverted from landfills
```{r}
