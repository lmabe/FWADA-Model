...
# Splitting clusters
If a cluster is over 140% full, it can be split equally with both new clusters above the 70% minimum operating capacity of the AD  
Clusters are split perpendicularly to the principal direction through the weighted center.  
If the split does not create relatively equal sized clusters, it will be moved incrementally towards the larger cluster until both clusters are equal (within 5% of the total FW within the cluster form one another). 
...
Principal Component Analysis (PCA) is used on the coordinates to determine the principal direction of the cluster. 
```{r}
...
The angle from the X axis (from East) is calculated for the principal direction (PD) (largest eigenvector).  
Other angles are determined by adding 90 degrees to this value.  

The weighted center of the cluster and the bounding box coordinates are also determined here.  
```{r}
...
Now that we have the angles of the eigenvectors, the weighted center, and the bbox, we can begin to split the cluster


## Establish eigenvector/bbox intersections
To split the cluster, a polygon that covers half the cluster is needed to use with the over() (spatial join) function.  

To draw the splitting polygon, we need to establish the where the eigenvector angles intersect the bbox.
These will be used as corner points for drawing the split_poly
```{r}
...
## Split the cluster
The cluster is split by building the split poly using 5 of the points established above.
These points include 2 bbox corners, the weighted center, and the endpoints of the line perpendicular to the principal direction
```{r}
...
In this case, the line needs to be moved towards the red cluster.  
This is line is moved until the cluster is split evenly.  

Note: This chunk just draws a new line (l). The real function modifies the split poly, resplits the cluster, and checks that is it even again.
```{r}
...
The other method of moving FW between clusters is the grabbing function which simply moves FW between adjacent ADs.  

This works by creating concentric circles from the AD of the cluster to grab FW.
The cumulative amount of FW is calculated for each ring, and FW points are reclassified to the grabbing cluster's AD code when it is full.  

(This method is inspired by Ripley's K function)  

## Sample clusters
Two sample clusters are needed in this example. Cluster x1 and x7 are adjacent.  
...
## Draw Circles, calculate cumulative FW
```{r}
...
## Reclassify points
The cutoff distance is determined by the capacity of the AD (shown in red on stepplot above).
Points within this distance get reclassified to the new AD
```{r}
...
The weighted center is used to determine new AD locations (not shown). 
In this example, there is very little of the blue cluster left. 
Most likely, its FW will be distributed to other adjacent ADs in later iterations and the blue cluster will be removed